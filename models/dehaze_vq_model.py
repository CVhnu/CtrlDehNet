...
from basicsr.archs.fourier_block_arch import GlobalNet_Fusion
...
class GaussianEncoder(nn.Module):
    def __init__(self, latent_dim=32, input_channels=3):
        ...
        Args:
            ...
            input_channels (int): 输入图像的通道数（默认3）
        """
        super().__init__()

        # 加载预训练的ResNet18，并取前4个block作为backbone
        ...
        self.backbone = nn.Sequential(
            ...
            resnet.relu,
            resnet.maxpool,
            resnet.layer1,  # 输出64通道
            resnet.layer2  # 输出128通道
        )
    ...
    def forward(self, x):
        """
        输入:
            x (Tensor): [B, C, H, W] 图像张量
        输出:
            mu (Tensor): [B, latent_dim] 高斯均值
            logvar (Tensor): [B, latent_dim] 对数方差
        """
        h = self.backbone(x)  # [B, 128, H/8, W/8]
        h = self.avgpool(h)  # [B, 128, 1, 1]
...
def kl_divergence(mu1, logvar1, mu2, logvar2):
    """
    计算两个高斯分布之间的KL散度 (KL(q1 || q2))
    公式: 0.5 * (logvar2 - logvar1 + (exp(logvar1) + (mu1 - mu2)^2) / exp(logvar2) - 1)
    """
...
class ContrastiveLoss(nn.Module):
    def __init__(self, lambda_close=1.0, lambda_far=0.5):
        """
        对比损失模块
        Args:
            lambda_close (float): 拉近GT的权重
            lambda_far (float): 推远LQ的权重
        """
        super().__init__()
        self.lambda_close = lambda_close
        self.lambda_far = lambda_far

    def forward(self, mu_pred, logvar_pred, mu_gt, logvar_gt, mu_lq, logvar_lq):
        """
        输入:
            mu/logvar_pred: 预测图的高斯参数
            mu/logvar_gt: GT图的高斯参数
            mu/logvar_lq: LQ图的高斯参数
        返回:
            contrastive_loss: 加权后的对比损失
        """
        # 拉近预测和GT (最小化KL(pred||gt))
        loss_close = kl_divergence(mu_pred, logvar_pred, mu_gt, logvar_gt).mean()

        # 推远预测和LQ (最大化KL(pred||lq)，因此取负号)
        loss_far = -kl_divergence(mu_pred, logvar_pred, mu_lq, logvar_lq).mean()

        # 加权求和
        return self.lambda_close * loss_close + self.lambda_far * loss_far
...
class VQDehazeModel(BaseModel):
    ...
    def init_training_settings(self):
        ...
        if self.opt['network_f']:
            self.net_f = build_network(self.opt['network_f'])
            self.net_f = self.model_to_device(self.net_f)
            load_path = self.opt['path'].get('pretrain_network_f', None)
            if load_path is not None:
                logger.info(f'Loading net_f from {load_path}')
                self.load_network(self.net_f, load_path)
            self.net_f.eval()
    ...
    def optimize_parameters(self, current_iter):
        ...
        # ===================================================
        # Gaussianspacial loss
        if train_opt.get('gaussian_opt', None):
            ge = GaussianEncoder().to(self.device)
            mu_gt, logvar_gt = ge(self.gt)
            mu_lq, logvar_lq = ge(self.lq)
            mu_pred, logvar_pred = ge(self.output_residual)
            contras = ContrastiveLoss().to(self.device)
            l_gaussian = contras(mu_gt=mu_gt, logvar_gt=logvar_gt, mu_lq=mu_lq, logvar_lq=logvar_lq, mu_pred=mu_pred,
                                 logvar_pred=logvar_pred)
            l_gaussian *= train_opt['gaussian_opt']['loss_weight']
            l_g_total += l_gaussian.mean()
            loss_dict['gaussian_opt'] = l_gaussian.mean()
    ...
    def test(self):
        ...
        stage3 =True
        if stage3 :
            net_f = self.get_bare_model(self.net_f)
            lq_input = net_f.test(lq_input)
        if h*w < min_size:
